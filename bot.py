...
def rewrite_news(title, body):
    """
    Переписывает заголовок и текст через cointegrated/rut5-base-paraphraser.
    """
    try:
        api_url = "https://api-inference.huggingface.co/models/cointegrated/rut5-base-paraphraser"
        ...
        new_title = paraphrase(title)
        # Ограничим текст для скорости
        body_part = body[:1200] if len(body) > 1200 else body
        new_body = paraphrase(body_part)
        ...
        if (SequenceMatcher(None, title, new_title).ratio() < 0.95 and
            ...
            print("Рерайт выполнен успешно")
            return new_title, new_body
        else:
            print("Рерайт не дал изменений, оставляем оригинал")
            return title, body
    except Exception as e:
